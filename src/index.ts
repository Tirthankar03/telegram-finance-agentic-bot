...
async function textToSpeech(
  ...
  const outputPath = path.join(
    __dirname,
    `voice_response_${chatId}_${Date.now()}.mp3`
  );
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      writer.on("error", (err) =>
        reject(new Error("Error writing audio file: " + err.message))
      );
    });
  } catch (error) {
...
async function get_current_date() {
  ...
  return {
    ...
    result: {
      ...
      formatted: today.toLocaleDateString("en-GB", {
        day: "2-digit",
        month: "2-digit",
        year: "numeric",
      }),
...
async function add_entry_to_sheet({
  amount,
  description,
  category,
  type,
  ...
  const values = [
    [monthNumeric, date, description, finalCategory, income, debits],
  ];
  ...
  try {
    ...
    let lastRow = lastRowResponse.data.values?.[0]?.[0]
      ? parseInt(lastRowResponse.data.values[0][0])
      : 1;
    ...
    if (
      type === "expense" &&
      monthNumeric === (new Date().getMonth() + 1).toString()
    ) {
      ...
      const budget =
        parseFloat(budgetValues[0][0].replace(/[^0-9.-]+/g, "")) || 0;
      const budgetLeft =
        parseFloat(budgetValues[1][0].replace(/[^0-9.-]+/g, "")) || 0;

      const percentageLeft = budget > 0 ? (budgetLeft / budget) * 100 : 0;

      if (percentageLeft < 25) {
        message += ` Your budget for this month is ₹${budget}, with ₹${budgetLeft} left (${percentageLeft.toFixed(
          2
        )}%). Be more careful and cautious about your spending.`;
...
async function get_all_entries() {
  try {
    ...
    const lastRow = lastRowResponse.data.values?.[0]?.[0]
      ? parseInt(lastRowResponse.data.values[0][0])
      : 1;
...
async function set_budget_for_month({ amount }: { amount: number }) {
  ...
  try {
    ...
    const message =
      `Set budget for this month to ₹${budget}. You have ₹${budgetLeft} left (${percentageLeft.toFixed(
        2
      )}%). ${warning}`.trim();
    return { success: true, message };
  } catch (error) {
    ...
    functionDeclarations: [
      {
        ...
        description:
          "Adds a new entry to the financial tracker Google Sheet within rows 2-954, updating last appended row in J18. For expenses in the current month, includes budget status if below 50% or 25%.",
        parameters: {
          type: "object",
          properties: {
            amount: {
            ...
            category: {
              ...
              description:
                "The category of the transaction (optional, will be inferred if omitted).",
            },
            type: {
              type: "string",
              enum: ["expense", "income"],
              description: "Whether it’s an expense or income.",
            },
            month: {
              type: "string",
              description:
                "The month as a number (1-12), defaults to current month if omitted.",
            },
          },
      ...
      },
      {
        name: "query_sheet",
        description:
          'Queries the financial tracker Google Sheet for category totals (e.g., Income, Debt Given, Debt Returned) in a specified month. Use this for summarized financial data, such as total debts owed to you or expenses in a given month. Month parameter accepts numbers (1-12) or names (e.g., "February").',
        parameters: {
          type: "object",
          properties: {
            month: {
              type: "string",
              description:
                'The month to query (e.g., "February" or "2"), defaults to current month if omitted.',
            },
          },
          required: [],
        },
      },
      {
        ...
        description:
          "Fetches all financial entries from the Google Sheet up to the last appended row.",
      },
      {
        name: "get_budget_status",
        description:
          "Fetches the budget and remaining amount for the current month, with spending warnings if below 50% or 25%.",
      },
      {
        name: "set_budget_for_month",
        description:
          "Sets the budget for the current month in the financial tracker Google Sheet and returns the updated budget status.",
        parameters: {
          type: "object",
          properties: {
            amount: {
              type: "number",
              description: "The budget amount to set for the current month.",
            },
          },
          required: ["amount"],
        },
      },
      {
        ...
        description:
          'Returns the current date, including day, month (1-12), year, and formatted string (DD/MM/YYYY). Use this to determine relative time periods like "last month" or "this month" in queries.',
      },
    ],
  },
];
...
// Hono Routes
app.get("/", (c) => {
  return c.text("Hello From Telegram Agentic Bot!");
});
...
app.post("/finance", async (c) => {
  ...
  try {
    ...
    while (true) {
      ...
      for (const call of functionCalls) {
        ...
        if (functions[name]) {
          ...
          console.log(
            "functionResult after executing in my code>>>>",
            functionResult
          );

          if (functionResult.success) {
            finalResponse = functionResult.result
              ? typeof functionResult.result === "string"
                ? functionResult.result
                : JSON.stringify(functionResult.result)
              : functionResult.message ||
                JSON.stringify(functionResult.entries);
          } else {
      ...
      if (functionResponses.length > 0 && !finalResponse.startsWith("Error")) {
        console.log(
          "after all calls, final functionResponses before sending to LLM?>>>>>",
          functionResponses
        );
        ...
        console.log(
          "response after final functionResponses query?>>>>>",
          response
        );
        ...
        if (!response.functionCalls || !response.functionCalls()) {
          ...
          console.log(
            "finalResponse after all the calls and passing functionResponses>>>>",
            finalResponse
          );
        }
      } else {
    ...
    if (!finalResponse && response.text()) {
      ...
      console.log(
        "no final response so just putting the initial response here>>>",
        finalResponse
      );
    }

    return c.json({
      response: finalResponse || "Sorry, I couldn’t process your request.",
    });
  } catch (error) {
...
async function downloadFile(
  ...
  try {
    ...
    throw new Error(
      "Error downloading voice file: " + (error as Error).message
    );
  }
}
...
async function convertToMp3(
  ...
  return new Promise((resolve, reject) => {
    ffmpeg(inputPath)
      ...
      .on("error", (err) =>
        reject(new Error("Error converting to MP3: " + err.message))
      )
      .save(outputPath);
  });
}
...
async function transcribeAudio(filePath: string): Promise<string> {
  try {
    ...
    throw new Error(
      "Error transcribing audio with Groq: " + (error as Error).message
    );
  } finally {
